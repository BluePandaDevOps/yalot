...
class WhoisXml_SubDomains(BaseInputSources):
    ...
    '''
    {
      ...
      '''
    def _domains_request_domains(self,domain):
    ...
    def _domains_request_subdomains(self,domain):
        url = f"{self.BASE_URL}"
        headers = {
            "Content-Type": "application/json",
        }
    ...
    def search_subdomains(self,domain):
        search_results = self._domains_request_subdomains(domain)
        if search_results:
            app_logger.log(logging.DEBUG,json.dumps(search_results))
            self.insert_input_data(search_results)
            
        search_results = self._domains_request_domains(domain)
        if search_results:
            app_logger.log(logging.DEBUG,json.dumps(search_results))
            self.insert_input_data(search_results)
